...
function updateProgress(){
	...
	document.getElementById("music-time").innerHTML=formatTime(rem.audio[0].currentTime)+"/"+formatTime(rem.audio[0].duration);
    // 同步歌词显示	
...
//删除列表里的一项的函数
function listDelete(no,list) {
    // 记录要播放的歌曲的id
    var tmpid = no;

    // 调试信息输出
    if(mkPlayer.debug) {
        console.log("删除了列表中的第 " + (no + 1) + " 首歌 " + musicList[rem.dislist].item[no].name);
    }
